...
(function(){
  ...
  // Auto-apply legacy background after DOM ready (opt-in)
  if(window.__USE_LEGACY_BG__ === true){
...
const PERM_KEYS = {
  ...
  systemControl: 'systemControl' // ⭐ 給 22.js 用
};

function normalizePerms(userObj){
  const role = userObj?.role || 'guest';
  const perms = userObj?.perms || {};
...
function canPerm(key){
  return window.__AUTHZ__?.perms?.[key] === true;
}

async function loadAuthz(uid){
  const snap = await db.ref(`users/${uid}`).once('value');
  const authz = normalizePerms(snap.val());
  window.__AUTHZ__ = authz;
  return authz;
}

// 掛到 LR namespace，給全站用
window.LR = Object.assign(window.LR || {}, {
  PERM_KEYS,
  loadAuthz,
  canPerm
});
  ...
  window.LR = Object.assign(window.LR || {}, { getDeviceId, listen, markDeviceSeen, applyBackground });
})();
